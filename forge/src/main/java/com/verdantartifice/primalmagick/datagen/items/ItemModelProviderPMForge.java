 ...
 * @author Daedalus4096
 */
public class ItemModelProviderPMForge extends ModelProvider<ItemModelBuilderPMForge> {
    ...
    public ItemModelProviderPMForge(PackOutput output, CompletableFuture<HolderLookup.Provider> lookupProvider, ExistingFileHelper exFileHelper) {
        super(output, Constants.MOD_ID, ITEM_FOLDER, ItemModelBuilderPMForge::new, exFileHelper);
    ...
    protected void registerModels(HolderLookup.Provider lookupProvider) {
        ...
        PixieItemForge.getAllPixies().forEach(this::pixieItem);
    ...
    private ItemModelBuilderPMForge builder(Item item) {
        return this.builder(this.key(item));
    }
    
    private ItemModelBuilderPMForge builder(Identifier loc) {
    ...
    public ItemModelBuilderPMForge basicItem(Item item) {
    ...
    private ItemModelBuilderPMForge itemWithParent(Item item, Item parent) {
        return this.itemWithParent(item, this.defaultModelLoc(parent));
    }
    
    private ItemModelBuilderPMForge itemWithParent(Item item, Identifier parent) {
        return this.builder(item).parent(this.existingModel(parent));
    }
    
    private ItemModelBuilderPMForge handheldItem(Item item) {
    ...
    private ItemModelBuilderPMForge tridentItem(TridentItem item) {
        ItemModelBuilderPMForge throwingModel = this.tridentThrowingModel(item);
        ItemModelBuilderPMForge inHandModel = this.tridentInHandModel(item, throwingModel);
    ...
    private ItemModelBuilderPMForge tridentInHandModel(TridentItem item, ModelFile throwingModel) {
    ...
    private ItemModelBuilderPMForge tridentThrowingModel(TridentItem item) {
    ...
    private ItemModelBuilderPMForge bowItem(BowItem item) {
    ...
    private ItemModelBuilderPMForge bowPullingModel(BowItem item, int stage) {
    ...
    private ItemModelBuilderPMForge fishingRodItem(FishingRodItem item) {
        return this.builder(item)
    ...
    private ItemModelBuilderPMForge fishingRodCastModel(FishingRodItem item) {
    ...
    private ItemModelBuilderPMForge shieldItem(ShieldItem item, Identifier particleTexture) {
        return this.builder(item)
                .parent(new ModelFile.UncheckedModelFile("builtin/entity"))
                .guiLight(GuiLight.FRONT)
    ...
    private ItemModelBuilderPMForge shieldBlockingModel(ShieldItem item, Identifier particleTexture) {
    ...
    private ItemModelBuilderPMForge armorItemWithTrims(ArmorItem item, List<ResourceKey<TrimMaterial>> trimKeys, HolderLookup.Provider lookupProvider) {
        return this.armorItemWithTrims(item, trimKeys.stream().map(key -> this.getTrimMaterial(key, lookupProvider)).toList());
    }
    
    private ItemModelBuilderPMForge armorItemWithTrims(ArmorItem item, List<Holder<TrimMaterial>> trims) {
        return this.armorItemWithTrims(item, this.getArmorTrimOverlay(item), trims);
    }
    
    private ItemModelBuilderPMForge armorItemWithTrims(ArmorItem item, Identifier trimOverlayLoc, List<Holder<TrimMaterial>> trims) {
        Identifier trimType = Identifier.withDefaultNamespace("trim_type");
        ItemModelBuilderPMForge builder = this.basicItem(item);
    ...
    private ItemModelBuilderPMForge trimmedArmorModel(ArmorItem item, Identifier trimOverlayLoc, Holder<TrimMaterial> trim) {
    ...
    private ItemModelBuilderPMForge itemWithOverlay(Item item) {
    ...
    private ItemModelBuilderPMForge spawnEggItem(SpawnEggItem item) {
        return this.itemWithParent(item, Identifier.withDefaultNamespace("item/template_spawn_egg"));
    }
    
    private void pixieItem(PixieItemForge item) {
    ...
    private ItemModelBuilderPMForge dyeableItem(Item item, DyeColor defaultColor) {
        Identifier key = this.key(item);
        ItemModelBuilderPMForge builder = this.builder(key)
                .parent(new ModelFile.UncheckedModelFile("item/generated"))
    ...
    private ItemModelBuilderPMForge dyedItemModel(Item item, DyeColor color) {
    ...
    private ItemModelBuilderPMForge pixieHouseItem(PixieHouseItem item, Identifier particleTexture) {
        return this.builder(item)
                .parent(new ModelFile.UncheckedModelFile("builtin/entity"))
                .guiLight(GuiLight.FRONT)
                .texture("particle", particleTexture)
