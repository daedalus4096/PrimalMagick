    ...
    public boolean isKeyDown(KeyMapping keybind) {
        if (keybind.isUnbound()) {
            ...
            boolean isDown = switch (keybind.getKey().getType()) {
                case KEYSYM -> InputConstants.isKeyDown(mc.getWindow(), keybind.getKey().getValue());
                case MOUSE -> GLFW.glfwGetMouseButton(mc.getWindow().handle(), keybind.getKey().getValue()) == GLFW.GLFW_PRESS;
                default -> false;
            };
