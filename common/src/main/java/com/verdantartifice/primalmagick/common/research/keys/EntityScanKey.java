...
public class EntityScanKey extends AbstractResearchKey<EntityScanKey> {
    public static final MapCodec<EntityScanKey> CODEC = Identifier.CODEC.fieldOf("entityType").xmap(loc -> {
        return new EntityScanKey(Services.ENTITY_TYPES_REGISTRY.get(loc));
    }, key -> {
        return EntityType.getKey(key.entityType);
    });
    public static final StreamCodec<ByteBuf, EntityScanKey> STREAM_CODEC = Identifier.STREAM_CODEC.map(loc -> {
        return new EntityScanKey(Services.ENTITY_TYPES_REGISTRY.get(loc));
    }, key -> {
        return EntityType.getKey(key.entityType);
    });
