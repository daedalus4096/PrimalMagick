...
public class BloodDamageSpellPayload extends AbstractDamageSpellPayload<BloodDamageSpellPayload> {
    ...
    public void playSounds(@NotNull Level world, @NotNull BlockPos origin) {
        world.playSound(null, origin, SoundsPM.BLOOD.get(), SoundSource.PLAYERS, 1.0F, 1.0F + (float)(world.getRandom().nextGaussian() * 0.05D));
    }
    ...
    protected float getBaseDamage(SpellPackage spell, @Nullable ItemStack spellSource, @Nullable LivingEntity caster, HolderLookup.Provider registries) {
        return 3.0F + (2.0F * this.getModdedPropertyValue(SpellPropertiesPM.POWER.get(), spell, spellSource, caster, registries));
    }
