...
public abstract class AbstractWandManaTest extends AbstractBaseTest {
    ...
    public Collection<TestFunction> wand_can_get_and_add_mana(String testName, String templateName) {
        return TestUtils.createParameterizedTestFunctions(testName, templateName, SOURCE_TEST_PARAMS, (helper, source) -> {
            ...
            IWand wand = this.assertInstanceOf(helper, wandStack.getItem(), IWand.class, "Wand stack is not a wand as expected");

            // Confirm that the wand is empty at first
            this.assertValueEqual(helper, wand.getMana(wandStack, source), 0, "Wand is not empty as expected");

            // Add a point of real mana to the wand
            this.assertValueEqual(helper, wand.addMana(wandStack, source, 1), 0, "Failed to add centimana to wand");

            // Confirm that the wand has mana in it
            this.assertValueEqual(helper, wand.getMana(wandStack, source), 1, "Wand mana total is not as expected");
    ...
    public Collection<TestFunction> wand_can_get_and_add_real_mana(String testName, String templateName) {
        return TestUtils.createParameterizedTestFunctions(testName, templateName, SOURCE_TEST_PARAMS, (helper, source) -> {
            ...
            // Confirm that the wand is empty at first
            this.assertTrue(helper, wand.getMana(wandStack, source) == 0, "Wand is not empty as expected");

            // Add a point of real mana to the wand
            this.assertTrue(helper, wand.addMana(wandStack, source, 100) == 0, "Failed to add real mana to wand");

            // Confirm that the wand has mana in it
            this.assertTrue(helper, wand.getMana(wandStack, source) == 100, "Wand mana total is not as expected");
    ...
    public Collection<TestFunction> wand_cannot_add_too_much_mana(String testName, String templateName) {
        return TestUtils.createParameterizedTestFunctions(testName, templateName, SOURCE_TEST_PARAMS, (helper, source) -> {
            ...
            this.assertValueEqual(helper, wand.getMana(wandStack, source), maxCentimana, "Wand mana total");
            this.assertValueEqual(helper, actualMana, expectedCentimana, "Wand overfill");
    ...
    public Collection<TestFunction> wand_can_get_all_mana(String testName, String templateName) {
        return TestUtils.createParameterizedTestFunctions(testName, templateName, SOURCE_TEST_PARAMS, (helper, source) -> {
            ...
            this.assertTrue(helper, wand.getAllMana(wandStack).equals(sourceList), "Wand mana total is not as expected");
    ...
    public Collection<TestFunction> wand_can_consume_mana(String testName, String templateName) {
        return TestUtils.createParameterizedTestFunctions(testName, templateName, SOURCE_TEST_PARAMS, (helper, source) -> {
            ...
            // Add a point of real mana to the wand
            this.assertTrue(helper, wand.addMana(wandStack, source, startingCentimana) == 0, "Failed to add real mana to wand");

            // Confirm that a few points of centimana can be consumed
            this.assertTrue(helper, wand.consumeMana(wandStack, player, source, consumedCentimana, helper.getLevel().registryAccess()), "Failed to consume mana from wand");

            // Confirm that the mana was deducted correctly
            var actual = wand.getMana(wandStack, source);
            this.assertValueEqual(helper, actual, expectedCentimana, "Mana total for " + source.getId());
    ...
    public Collection<TestFunction> wand_cannot_consume_more_mana_than_it_has(String testName, String templateName) {
        return TestUtils.createParameterizedTestFunctions(testName, templateName, SOURCE_TEST_PARAMS, (helper, source) -> {
            ...
            this.assertFalse(helper, wand.consumeMana(wandStack, player, source, 200, helper.getLevel().registryAccess()), "Consumption of maan succeeded when it shouldn't have");
    ...
    public Collection<TestFunction> wand_can_consume_multiple_types_of_mana(String testName, String templateName) {
        return TestUtils.createParameterizedTestFunctions(testName, templateName, SOURCE_TEST_PARAMS, (helper, source) -> {
            ...
            Sources.getAll().forEach(s -> {
                this.assertTrue(helper, wand.addMana(wandStack, s, startingCentimana) == 0, "Failed to add real mana to wand for " + s.getId());
            });
            ...
            this.assertTrue(helper, wand.consumeMana(wandStack, player, sourceList, helper.getLevel().registryAccess()), "Failed to consume mana from wand");
            ...
            Sources.getAll().forEach(s -> {
                ...
                this.assertValueEqual(helper, actual, expected, "Mana total for " + s.getId());
            });
    ...
    public Collection<TestFunction> wand_cannot_consume_more_mana_than_it_has_with_multiple_types(String testName, String templateName) {
        return TestUtils.createParameterizedTestFunctions(testName, templateName, SOURCE_TEST_PARAMS, (helper, source) -> {
            ...
            this.assertFalse(helper, wand.consumeMana(wandStack, player, sourceList, helper.getLevel().registryAccess()), "Mana consumption succeeded when it shouldn't have");

            // Confirm that the wand's mana is still in its original state
            Sources.getAll().forEach(s -> this.assertTrue(helper, wand.getMana(wandStack, source) == 100, "Mana total is not as expected"));
    ...
    public Collection<TestFunction> wand_can_remove_mana_raw(String testName, String templateName) {
        return TestUtils.createParameterizedTestFunctions(testName, templateName, SOURCE_TEST_PARAMS, (helper, source) -> {
            ...
            this.assertTrue(helper, wand.addMana(wandStack, source, startingCentimana) == 0, "Failed to add mana to wand");

            // Confirm that a few points of centimana can be consumed
            this.assertTrue(helper, wand.removeManaRaw(wandStack, source, removedCentimana), "Failed to remove mana from wand");

            // Confirm that the mana was deducted correctly
            this.assertTrue(helper, wand.getMana(wandStack, source) == expectedCentimana, "Wand mana total is not as expected");
    ...
    public Collection<TestFunction> wand_cannot_remove_more_raw_mana_than_it_has(String testName, String templateName) {
        return TestUtils.createParameterizedTestFunctions(testName, templateName, SOURCE_TEST_PARAMS, (helper, source) -> {
            ...
            // Add some real mana to the wand for the test source
            this.assertTrue(helper, wand.addMana(wandStack, source, 100) == 0, "Failed to add mana to wand");

            // Confirm that attempting to remove more than that fails
            this.assertFalse(helper, wand.removeManaRaw(wandStack, source, 200), "Mana removal succeeded when it shouldn't have");

            // Confirm that the wand's mana is still in its starting state
            this.assertTrue(helper, wand.getMana(wandStack, source) == 100, "Mana total is not as expected");
    ...
    public Collection<TestFunction> wand_contains_mana(String testName, String templateName) {
        return TestUtils.createParameterizedTestFunctions(testName, templateName, SOURCE_TEST_PARAMS, (helper, source) -> {
            ...
            this.assertTrue(helper, overflow == 0, "Failed to add real mana to wand, overflow is " + overflow);

            // Confirm that the wand recognizes it contains centimana up to the threshold of what it was given
            this.assertValueEqual(helper, wand.getMana(wandStack, source), startingCentimana, "Mana total for source " + source.getId());
            this.assertTrue(helper, wand.containsMana(wandStack, player, source, lessCentimana, helper.getLevel().registryAccess()), "Contains returned false for less than held");
            this.assertTrue(helper, wand.containsMana(wandStack, player, source, exactCentimana, helper.getLevel().registryAccess()), "Contains returned false for exact held");
            this.assertFalse(helper, wand.containsMana(wandStack, player, source, greaterCentimana, helper.getLevel().registryAccess()), "Contains returned true for greater than held");
    ...
    public Collection<TestFunction> wand_contains_mana_list(String testName, String templateName) {
        return TestUtils.createParameterizedTestFunctions(testName, templateName, SOURCE_TEST_PARAMS, (helper, source) -> {
            ...
            this.assertTrue(helper, wand.containsMana(wandStack, player, greenList, helper.getLevel().registryAccess()), "Contains returned false for green list");
            ...
            this.assertFalse(helper, wand.containsMana(wandStack, player, redList, helper.getLevel().registryAccess()), "Contains returned true for red list");
    ...
    public Collection<TestFunction> wand_contains_mana_raw(String testName, String templateName) {
        return TestUtils.createParameterizedTestFunctions(testName, templateName, SOURCE_TEST_PARAMS, (helper, source) -> {
            ...
            this.assertTrue(helper, overflow == 0, "Failed to add mana to wand, overflow is " + overflow);

            // Confirm that the wand recognizes it contains centimana up to the threshold of what it was given
            this.assertTrue(helper, wand.containsManaRaw(wandStack, source, lessCentimana), "Contains returned false for less than held");
            this.assertTrue(helper, wand.containsManaRaw(wandStack, source, exactCentimana), "Contains returned false for exact held");
            this.assertFalse(helper, wand.containsManaRaw(wandStack, source, greaterCentimana), "Contains returned true for greater than held");
