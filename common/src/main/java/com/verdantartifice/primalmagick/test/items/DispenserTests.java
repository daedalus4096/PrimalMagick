...
public class DispenserTests extends AbstractBaseTest {
    public static void mana_arrows_fired_from_dispenser(GameTestHelper helper, ManaArrowItem arrow) {
        ...
        helper.succeedWhen(() -> {
            ...
            helper.getEntities(EntityTypesPM.MANA_ARROW.get(), expectedArrowPos, 1D).forEach(e -> {
                assertTrue(helper, e.getSource().equals(arrow.getSource()), "Arrow entity source is not as expected: " + e.getSource());
            });
        });
    }
}
