...
public class RitualEnchantmentTests extends AbstractBaseTest {
    public static void enchantment_essence_thief(GameTestHelper helper, int enchLevel) {
        ...
        assertTrue(helper, actualLoot.size() == 1, "Modified loot does not contain exactly one item");
        var stack = actualLoot.getFirst();
        assertTrue(helper, stack.getCount() == 1, "Modified loot stack does not have a count of one");
        ...
        assertTrue(helper, essenceItem.getEssenceType().equals(expectedEssenceType), "Modified loot essence is not of expected grade");
        assertTrue(helper, essenceItem.getSource().equals(Sources.BLOOD), "Modified loot essence is not of expected source");
