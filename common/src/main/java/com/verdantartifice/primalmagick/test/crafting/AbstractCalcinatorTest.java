...
public class AbstractCalcinatorTest extends AbstractBaseTest {
    public void calcinator_works(GameTestHelper helper) {
        ...
        this.assertTrue(helper, calcinator.getItem(0, 0).is(Items.COBBLESTONE), "Input cobblestone not set correctly");
        calcinator.setItem(1, 0, new ItemStack(Items.CHARCOAL));
        this.assertTrue(helper, calcinator.getItem(1, 0).is(Items.CHARCOAL), "Input fuel not set correctly");
        for (int index = 0; index < 9; index++) {
            this.assertTrue(helper, calcinator.getItem(2, index).isEmpty(), "Output slot " + index + " is not empty before calcination");
        }
        ...
        this.assertTrue(helper, calcinator.getItem(0, 0).isEmpty(), "Input item stack not empty after calcination");
        var outputStack = calcinator.getItem(2, 0);
        this.assertTrue(helper, outputStack.is(ItemsPM.ESSENCE_DUST_EARTH.get()), "Output is not Earth Dust as expected");
        this.assertTrue(helper, outputStack.getCount() == 1, "Output stack count is not one as expected");
