...
public class LanguageProviderEnUs extends AbstractLanguageProviderPM {
    ...
    protected void addLocalizations(HolderLookup.Provider lookupProvider) {
        ...
        this.label("topic_link").sub("text").sub("default").output("Take me there!").end().build();
        this.label("topic_link").sub("text").sub("sotu").output("Secrets of the Universe?").end().build();
        ...
        this.advancement("large_network_siphon").name("Link Saturation").description("Siphon at least 256 mana into a magitech device in a single instant").build();
