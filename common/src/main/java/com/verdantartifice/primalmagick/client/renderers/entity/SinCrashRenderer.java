...
public class SinCrashRenderer extends EntityRenderer<SinCrashEntity> {
    ...
    public void render(SinCrashEntity entity, float entityYaw, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int packedLight) {
        ...
        int coreColor = ARGB.color(ARGB.as8BitChannel(1.0F), Sources.VOID.getColor());
        int glowColor = ARGB.color(ARGB.as8BitChannel(0.5F), Sources.VOID.getColor());
    ...
    @Override
    public Identifier getTextureLocation(SinCrashEntity entity) {
        return TEXTURE;
    }
}
