...
public class ThickLinesRenderType extends RenderType {
    ...
    public static final RenderType THICK_LINES = makeType("thick_lines", DefaultVertexFormats.POSITION_COLOR, GL11.GL_LINES, 256,
            RenderType.State.getBuilder().line(THICK_LINE_STATE).layer(VIEW_OFFSET_Z_LAYERING).transparency(TRANSLUCENT_TRANSPARENCY).writeMask(COLOR_WRITE).build(false));
